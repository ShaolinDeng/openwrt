...
#define SEL_LOAD_FIRMWARE_WRITE_FLASH   102 /* f */
...
void OperationSelect(void)
{
	...
	printf("   %c: Load firmware then write to Flash via TFTP. \n", SEL_LOAD_FIRMWARE_WRITE_FLASH);
}
...
int tftp_config(int type, char *argv[])
{
    ...
    else if (type == SEL_LOAD_FIRMWARE_WRITE_FLASH) {
#if defined (RT2880_ASIC_BOARD) || defined (RT2880_FPGA_BOARD)
		argv[1] = "0x8a100000";
#else
		argv[1] = "0x80100000";
#endif
		printf("\tInput Firmware filename ");
		//argv[2] = "uImage"; winfred: use strncpy instead to prevent the buffer overflow at copy_filename later
		strncpy(argv[2], "factory.bin", ARGV_LEN);
	}
...
#define MTK_I2C_TIME_US         10
#define MTK_I2C_WAIT_TIMES      100
#define MTK_I2C_BIT(nr)     (1UL << (nr))
static int mtk_i2c_wait_idle(void)
{
    int i=0;
    u32 status = 0x00;
    ...
    while(1)
    {
        status = RALINK_REG(0xb0000944);
        if(!(status & MTK_I2C_BIT(0)))
            break;
        i++;
        if(i > 100)
            return -1;
        udelay(MTK_I2C_TIME_US);
    }
    return 0;
}
static int mtk_i2c_check_ack(u32 expected)
{
    u32 ack = RALINK_REG(0xb0000900 + 0x44);
    u32 ack_expected = (expected << 16) & (0xFF << 16);
    return ((ack & ack_expected) == ack_expected) ? 0 : -1;
}
static int mtk_i2c_wirte_to_slave_device(u32 addr, u32 data1, u32 data2, u32 page_len, u32 value)
{
    if(mtk_i2c_wait_idle())
    {
         printf("wait hardware idle error!\n");
         goto our_error;
    }
    //start
    RALINK_REG(0xb0000944) = 0x00000011;
    if(mtk_i2c_wait_idle())
    {
         printf("wait start idle error!\n");
         goto our_error;
    }
    RALINK_REG(0xb0000950) = addr;
    RALINK_REG(0xb0000944) = 0x00000021;
    if(mtk_i2c_wait_idle())
    {
         printf("wait write idle error!\n");
         goto our_error;
    }
    if(mtk_i2c_check_ack(MTK_I2C_BIT(0)))
    {
        printf("check_ack error!\n");
        goto our_error;
    }
    RALINK_REG(0xb0000950) = data1;
    RALINK_REG(0xb0000954) = data2;

    RALINK_REG(0xb0000944) = value;
    if(mtk_i2c_wait_idle())
    {
        printf("wait cmd idle error!\n");
        goto our_error;
    }

    if(mtk_i2c_check_ack((1<<page_len)-1))
    {
        printf("check_ack write data error!\n");
        goto our_error;
    }

    //stop
    RALINK_REG(0xb0000944) = 0x00000031;
    if(mtk_i2c_wait_idle())
    {
        printf("wait stop idle error!\n");
        goto our_error;
    }
...
static int mtk_i2c_init(void)
{
    printf("mtk i2c init!\n");
    RALINK_REG(0xb0000940) = 0x818f0003;
    RALINK_REG(0xb0000928) = 0x0;
    udelay(MTK_I2C_TIME_US*2000);
    return 0;
}
static int mtk_set_pca9557_for_led(void)
{
    int ret = 0;
    printf("PCA9557 led init!\n");

    // 初始化PCA9557外设芯片并设置 三色led灯、按键led灯 对应GPIO管脚的属性
    ret = mtk_i2c_wirte_to_slave_device(0x0000003e,0x80473001,0x801a50d8,0x00000002,0x00000121);
    if(0 != ret)
    {
        printf("mtk_i2c_wirte_to_slave_device error1!\n");
        goto our_error;
    }
    ret = mtk_i2c_wirte_to_slave_device(0x0000003e,0x80404703,0x801ad154,0x00000002,0x00000121);
    if(0 != ret)
    {
        printf("mtk_i2c_wirte_to_slave_device error2!\n");
        goto our_error;
    }
    ret = mtk_i2c_wirte_to_slave_device(0x0000003e,0x80473801,0x801a50d8,0x00000002,0x00000121);
    if(0 != ret)
    {
        printf("mtk_i2c_wirte_to_slave_device error3!\n");
        goto our_error;
    }
    // 设置三色led灯为蓝色并关闭按键led灯
    ret = mtk_i2c_wirte_to_slave_device(0x0000003e,0x87da1801,0x00000000,0x00000002,0x00000121);
    if(0 != ret)
    {
        printf("mtk_i2c_wirte_to_slave_device error4!\n");
        goto our_error;
    }
    return 0;
...
/*config bootdelay via environment parameter: bootdelay */
	...
	while (timer1 > 0) {
		...
		for (i=0; i<100; ++i) {
			if ((my_tmp = tstc()) != 0) {	/* we got a key press	*/
				...
				BootType = getc();
				if ((BootType < '0' || BootType > '5') && (BootType != '7') && (BootType != '8')
                    && (BootType != '9') && (BootType != SEL_LOAD_FIRMWARE_WRITE_FLASH))
					BootType = '3';
				printf("\n\rYou choosed %c\n\n", BootType);
				break;
			}
	...
	if(BootType == '3') {
...
#elif defined (CFG_ENV_IS_IN_SPI)
			if (1) {
                    printf("define CFG_ENV_IS_IN_SPI!\n");
				unsigned int load_address = simple_strtoul(argv[1], NULL, 16);
        ...
        case SEL_LOAD_FIRMWARE_WRITE_FLASH:
                    printf("   \n%c: System Load the full-firmware then write to Flash via TFTP. \n", SEL_LOAD_FIRMWARE_WRITE_FLASH);
                    printf(" Warning!! Erase the whole Flash then burn new one. Are you sure?(Y/N)\n");
                    confirm = getc();
                    if (confirm != 'y' && confirm != 'Y') {
                        printf(" Operation terminated\n");
                        break;
                    }
                    tftp_config(SEL_LOAD_FIRMWARE_WRITE_FLASH, argv);
                    argc= 3;
                    setenv("autostart", "no");
                    do_tftpb(cmdtp, 0, argc, argv);
#if defined (CFG_ENV_IS_IN_NAND)
                    if (1) {
                        printf("***********************************\n");
                        printf("Unsupport flash type now !! \n");
                        printf("***********************************\n");
                    }
#elif defined (CFG_ENV_IS_IN_SPI)
                    if (1) {
                        unsigned int load_address = simple_strtoul(argv[1], NULL, 16);
                        raspi_erase_write_all((u8 *)load_address, 0, NetBootFileXferSize);
                    }
#else //CFG_ENV_IS_IN_FLASH
                    if (1) {
                        printf("***********************************\n");
